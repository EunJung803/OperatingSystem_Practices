...
int mul_inv(int a, int m)
{
    ...
    if(d_1 == 1) //서로소 인가
        return (x_1>0?x_1:x_1+m); //항상 x_1 양수 값
    else
        return 0;
}
...
uint8_t umul_inv(uint8_t a, uint8_t m)
{
    /* Step X1. Initialise */
    uint8_t d0 = a, d1 = m;
    uint8_t x0 = 1, x1 = 0;
...
int main(void)
{
    ...
    /*——추가 부분——*/
    //a=1~255 검사 코드 a의 역원이 없으면 none이 나온다.
    for (a = 1 ; a < 256; a++) {
        m = 9223372036854775808u;
        ai = umul_inv(a, m);
        
        //printf("a = %d, m = %llu, ", a, m);
        //printf("a^-1 mod m = %llu\n", ai);
        printf("a = %d, m = %llu, a^-1 mod m = ", a, m);
        if (ai == 0) printf("none\n");
        else printf("%llu\n", ai);
    }
    /*——추가 부분 끝——*/
